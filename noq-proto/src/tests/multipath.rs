...
fn throughput() -> TestResult {
    ...
    bytes_to_send -= pair
        ...
        .write(&ZEROES)?;
    pair.drive_client(); // generates the datagram
    pair.advance_time(); // waits for latency for the datagram to be sent
    pair.drive_server(); // receives the datagram
    let server_stream = pair.conn_mut(Server).streams().accept(Dir::Bi).unwrap();
    loop {
        if bytes_to_send > 0 {
            send_bytes(
